...
export function ReconstructionPanel({ captureSessionId, setError, onModelApproved }: Props) {
  ...
  return <section className="reconstruction-panel" aria-live="polite">
    ...
    {artifact && (artifact.observed_ratio !== null || artifact.inferred_ratio !== null) ? <div className="photo-capture-note"><CircleAlert size={16} aria-hidden="true" /><div><strong>Origem da forma</strong><p>Partes observadas nas fotos: {Math.round((artifact.observed_ratio ?? 0) * 100)}%. Partes estimadas pelo processamento: {Math.round((artifact.inferred_ratio ?? 0) * 100)}%. As correções posteriores aparecem no histórico abaixo.</p></div></div> : artifact ? <div className="photo-capture-note"><CircleAlert size={16} aria-hidden="true" /><div><strong>Origem das partes não identificada</strong><p>Este processador não separou quais regiões vieram das fotos e quais foram estimadas. Compare toda a forma com o objeto real antes de aprovar.</p></div></div> : null}
    {artifact && qualification ? <React.Suspense fallback={<p className="photo-capture-note">Carregando preparação segura…</p>}><MeshRepairPanel jobId={job.id} rawUnit={artifact.unit} rawChecks={qualification.report.checks ?? {}} rawDimensions={qualification.report.dimensions} rawTriangles={qualification.report.preview_triangles} setError={setError} onModelApproved={onModelApproved} /></React.Suspense> : null}
  </section>;
}
