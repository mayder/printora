...
export function MeshRepairPanel({ jobId, rawUnit, rawChecks, setError }: Props) {
  ...
  async function create(operation: MeshRepairOperation, outputFormat: "obj" | "stl" | "3mf") {
    setBusy(true);
    setError(null);
    try {
  ...
  return <div className="mesh-repair-panel" aria-labelledby="mesh-repair-title">
    ...
    {!active && !recommendation && finalBlocker ? <div className="mesh-repair-export"><CircleAlert size={18} aria-hidden="true" /><div><strong>O arquivo final ainda está bloqueado</strong><p>{finalBlocker}</p></div></div> : null}
