...
assert.match(navigationSource, /\{ title: "Principal", sections: \["overview", "onboarding", "printers", "agents", "projects", "materials", "social", "catalog", "setup"\] \}/);
...
assert.match(appHookSource, /user\.platform_admin/);
assert.match(appHookSource, /isPlatformAdmin && !selfUpdate\.systemReleases/);
assert.match(settingsScreenSource, /authUser\?\.platform_admin === true/);
