...
MODE="plan"
NODE_MAJOR="${PRINTORA_NODE_MAJOR:-22}"
...
env_file=$ENV_FILE
action=$(if [[ "$node_supported" == "true" && -n "$npm_path" ]]; then echo "usar Node atual"; else echo "instalar Node $NODE_MAJOR via nvm para o Printora"; fi)
PLAN
