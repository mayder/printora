...
backlog="$(toml_string_value files backlog)"
[[ "$backlog" == "DEMANDAS.md" || "$backlog" == "demandas.md" ]] || fail "PATHS.toml sem files.backlog correto"
...
while IFS= read -r file; do [[ -z "$file" ]] && continue; path="${file#./}"; [[ -e "$path" ]] || fail "PATHS.toml referencia caminho inexistente: $file"; done < <(toml_array_values checks required)
