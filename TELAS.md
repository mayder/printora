...
- Na aba Operacao do detalhe da impressora, o card `Impressão` pode exibir a thumbnail do G-code retornada pelo Moonraker e uma prévia isométrica da camada atual gerada pelo agente a partir do G-code, com controles locais de rotação da prévia. A prévia deve ser compacta/cacheada e nunca bloquear operação, comando protegido ou impressão quando o arquivo for grande ou a leitura falhar.
...
- Na aba Operacao do detalhe da impressora, o card `Impressão` fica em faixa propria e os blocos `Temperaturas` e `Machine` ficam abaixo em colunas independentes; nenhum card deve ser esticado para acompanhar a altura de outro card e criar area vazia interna.
