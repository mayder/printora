...
- Implementação atual: a área `Projetos de impressão` existe no menu principal com contrato canônico, busca pública, filtros por tipo/licença/origem, estado vazio, cards, detalhe central, arquivos, versões/snapshots, comunidades, distinção entre projeto hospedado e referência externa e ação `Salvar nos meus projetos` como referência sem cópia. `Meus projetos`, upload pessoal, publicação comercial e jobs operacionais entram nas próximas entregas.
