...
- Na aba Operacao do detalhe da impressora, o card `Impressão` fica em faixa propria, usa a previsualizacao 3D como area principal e coloca thumbnail, fatos da impressao e limites de `Machine` na lateral direita quando houver largura. `Temperaturas` fica abaixo em largura cheia. Nenhum card deve ser esticado para acompanhar a altura de outro card ou criar area vazia interna.
