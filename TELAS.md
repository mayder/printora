...
- Enquanto o agente estiver offline, a SPA revalida sua presenca a cada 10 segundos. Quando o agente estiver online mas o Moonraker ainda nao estiver confirmado, health e operacao sao revalidados sequencialmente a cada 3 segundos; a tela nunca presume Moonraker online apenas pelo heartbeat do agente.
- Ao abrir o detalhe da impressora, o health rapido precede consultas operacionais mais pesadas para atualizar primeiro o estado de conexao.
...
| Digitalizar objeto | estado dedicado em `projects` | Entrada `Digitalizar este objeto` no detalhe de projeto em `Meus projetos` | `frontend/src/screens/projects/PhotoCapturePanel.tsx` e guia visual `CapturePositionGuide`; etapas posteriores permanecem separadas por responsabilidade | Cadastro guiado em quatro etapas; preparação com objeto de exemplo, oito posições ao redor e três alturas; 24 campos individuais agrupados em três conjuntos de oito; captura por câmera/arquivo, qualidade, escala, retomada e exportação privada | Nao exige impressora; somente a etapa opcional de fatiamento exige escolha posterior | existente |
| Revisar modelo criado por fotos | estado dedicado em `projects` | Após a reconstrução dentro de `Digitalizar objeto` e, no piloto, no histórico do projeto | `ReconstructionPanel`, `MeshRepairPanel` e histórico em `PrintProjectsScreen`; aprovação atualiza o projeto e aponta para a seção separada de fatiamento | Explica observado/inferido/reparado, recomenda uma correção por vez, exige comparação humana e medida, permite rejeitar ou aprovar STL/3MF, oferece `Continuar para o fatiamento` sem iniciar job ou impressão e, após imprimir, registra instrumento e medidas X/Y/Z | Uso mecânico e topologia/escala inseguras ficam bloqueados; impressora, material, perfil, prévia e preflight continuam obrigatórios; sucesso físico acima de 3% de desvio é recusado | existente |
...
| Atualizacoes da impressora | `frontend/src/components/modals/UpdateDialogModal.tsx` | Confirmacao, autorização contextual por senha/2FA, bloqueio de risco, rollback e progresso de update via Moonraker |
...
- O protocolo padrão solicita exatamente 24 posições: oito na altura do objeto,
  ...
  sessão.
- Antes do primeiro envio, a tela deve demonstrar o protocolo com um objeto de
  exemplo central, as oito posições numeradas e a altura da câmera. Durante a
  captura, todos os 24 campos devem continuar visíveis em três grupos de oito,
  cada um com direção, inclinação, estado e ação própria. A prevenção de erro
  pode liberar uma posição por vez, mas nunca deve esconder da pessoa a lista
  completa de imagens exigidas.
...
- Na aba Operacao do detalhe da impressora, o cache do preview deve preservar nomes Unicode usando o nome canônico do job autenticado, sem depender da fidelidade de um cabeçalho HTTP. Falhas recuperáveis de cache ou gateway devem repetir automaticamente com espera progressiva e jobs equivalentes coalescidos, mostrando estado de recuperação sem bloquear ou repetir a impressão.
...
- Na primeira leitura da aba Operacao, a UI deve mostrar estado de carregamento e reutilizar qualquer requisicao equivalente em andamento. Versao ainda nao recebida nao pode ser apresentada como agente desatualizado; o aviso de update exige uma versao instalada conhecida e inferior ao minimo.
...
- Textos de estado voltados ao usuário descrevem a ação ou consequência sem
  expor termos internos de desenvolvimento como endpoint, fixture, linguagem,
  banco de dados ou entregas futuras de pacote.
...
- O cabeçalho e as nove abas do detalhe da impressora devem permanecer dentro
  ...
  manutenção, firmware e visão geral seguem a mesma regra. Cada aba precisa ser
  visível, acionável e indicar sua seleção.
