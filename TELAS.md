...
- A página `/c/{slug}` possui abas: `Feed`, `Arquivos`, `Mods`, `Perfis`, `Membros` e `Impressoras públicas`. Feed consome `/api/social/communities/{slug}/feed` com ordenação, filtros técnicos, paginação e estados controlados; arquivos permanecem reservados para a biblioteca dedicada; mods usam os mods públicos declarados nas impressoras até existir biblioteca dedicada.
