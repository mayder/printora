...
- Na aba Operacao do detalhe da impressora, paridade visual com o G-code Viewer do Mainsail exige um contrato separado para cachear/servir o G-code completo e a posição `file_position` atual. O endpoint de status do agente continua leve e não deve transportar o arquivo completo dentro do resultado periódico.
...
- Na aba Operacao do detalhe da impressora, controles de rotação, zoom, pan e navegador 3D devem ocupar areas reservadas dentro da prévia, sem cobrir labels, contadores, thumbnail ou dados da impressão; em larguras menores, o card `Impressão` empilha visual, fatos e Machine sem criar buracos grandes.
