...
- Na aba Operacao do detalhe da impressora, o card `Impressão` pode exibir a thumbnail do G-code retornada pelo Moonraker e uma prévia navegável da camada atual gerada pelo agente a partir do G-code cacheado. A prévia deve mostrar a camada atual e o material já impresso abaixo dela, com geometria compacta para rotação/zoom locais no navegador, sem recalcular pesado a cada atualização do agente e sem bloquear operação, comando protegido ou impressão quando o arquivo for grande ou a leitura falhar.
- Na aba Operacao do detalhe da impressora, a prévia 3D do G-code deve preservar, quando o agente retornar, a classificação compacta das linhas de extrusão para diferenciar perímetro, preenchimento, superfície, suporte e saia/brim; camadas futuras podem aparecer como volume translúcido aproximado, sem prometer fidelidade completa de slicer.
...
- Na aba Operacao do detalhe da impressora, controles de rotação, zoom e navegador 3D devem ocupar areas reservadas dentro da prévia, sem cobrir labels, contadores, thumbnail ou dados da impressão; em larguras menores, o card `Impressão` empilha visual, fatos e Machine sem criar buracos grandes.
