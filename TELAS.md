...
- A tela Catálogo deve exibir fabricante, logo/monograma, resumo, site, repositório, documentação, Discord e Reddit quando disponíveis. O detalhe do modelo deve exibir página do modelo, Git do projeto, documentação, BOM, canais de comunidade, descrição, notas de curadoria, variações técnicas, volume útil, cinemática, firmware, componentes e `trust_state`. Identificadores internos de pacote ou campos de origem técnica não devem aparecer para o usuário.
