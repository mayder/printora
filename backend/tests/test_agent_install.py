...
def test_agent_install_plan_generates_single_use_token_and_command(tmp_path: Path, monkeypatch) -> None:
    ...
    try:
        ...
        with TestClient(app) as client:
            ...
            assert "PRINTORA_AGENT_VERSION='0.1.28'" in plan["install_command"]
            ...
            exchanged = client.post(
                ...
                json={"pairing_token": token, "stable_id": "agent-install-001", "agent_version": "0.1.28"},
            )
            assert exchanged.status_code == 200
            reused = client.post(
                "/api/agent/pairing/exchange",
                json={"pairing_token": token, "stable_id": "agent-install-002", "agent_version": "0.1.28"},
            )
    ...
    try:
        ...
        with TestClient(app) as client:
            ...
            credential = client.post(
                "/api/agent/pairing/exchange",
                json={"pairing_token": token, "stable_id": "agent-install-status", "agent_version": "0.1.28"},
            ).json()["credential"]
            ...
            heartbeat = client.post(
                "/api/agent/heartbeat",
                json={"agent_version": "0.1.28", "platform": "linux/arm64", "capabilities": {"installer": True}},
            ...
            assert ready["latest_version"] == "0.1.28"
