...
def test_agent_install_plan_generates_single_use_token_and_command(tmp_path: Path, monkeypatch) -> None:
    ...
    try:
        ...
        with TestClient(app) as client:
            ...
            assert "PRINTORA_AGENT_VERSION='0.1.36'" in plan["install_command"]
            assert "PRINTORA_AGENT_BIN_URL='http://testserver/api/agent/update/releases/0.1.36/linux-arm64'" in plan["install_command"]
            assert "PRINTORA_AGENT_SHA256='ffcddf95" in plan["install_command"]
            ...
            exchanged = client.post(
                ...
                json={"pairing_token": token, "stable_id": "agent-install-001", "agent_version": "0.1.36"},
            )
            assert exchanged.status_code == 200
            reused = client.post(
                "/api/agent/pairing/exchange",
                json={"pairing_token": token, "stable_id": "agent-install-002", "agent_version": "0.1.36"},
            )
    ...
    try:
        ...
        with TestClient(app) as client:
            ...
            credential = client.post(
                "/api/agent/pairing/exchange",
                json={"pairing_token": token, "stable_id": "agent-install-status", "agent_version": "0.1.36"},
            ).json()["credential"]
            ...
            heartbeat = client.post(
                "/api/agent/heartbeat",
                json={"agent_version": "0.1.36", "platform": "linux/arm64", "capabilities": {"installer": True}},
            ...
            assert ready["latest_version"] == "0.1.36"
...
def test_agent_installer_verifies_release_checksum_and_signature(tmp_path: Path) -> None:
    ...
    artifact = release_dir / "printora-agent-linux-arm64-0.1.36"
    metadata = json.loads(
        (release_dir / "printora-agent-linux-arm64-0.1.36.metadata.json").read_text()
    )
    env = {
        **os.environ,
