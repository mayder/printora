...
ON cloud_objects(owner_user_id, state, updated_at);

CREATE INDEX IF NOT EXISTS idx_cloud_objects_checksum
ON cloud_objects(sha256, size_bytes);

CREATE TABLE IF NOT EXISTS cloud_object_references (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
...
ON cloud_object_references(object_id, reference_type);

CREATE TABLE IF NOT EXISTS cloud_object_upload_sessions (
...
ON cloud_object_upload_sessions(state, expires_at);

CREATE TABLE IF NOT EXISTS cloud_object_reconciliation_runs (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    mode TEXT NOT NULL,
